  ...
  const runTest = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || "Gemini API test failed")
      }
      ...
      <CardContent className="space-y-4">
        ...
        {testStatus === "success" && testResult && (
          <div className="p-3 bg-green-50 border border-green-200 rounded-md">
            <p className="text-sm text-green-600 font-medium">✅ Gemini API is working correctly!</p>
            <div className="mt-2 text-xs text-gray-600">
              ...
              <p>Model: {testResult.model || "gemini-2.0-flash-exp"}</p>
              <p>
                Features:{" "}
                {[
        ...
        <div className="text-xs text-gray-500">
          <p>This test sends a short silent audio file to verify the Gemini API connection and configuration.</p>
        </div>
      </CardContent>
    </Card>
  )
